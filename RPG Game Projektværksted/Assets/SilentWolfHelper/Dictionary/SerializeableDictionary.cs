...
namespace SilentWolfHelper.Dictionary
{
    ...
    public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        public SerializableDictionary(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer = null) : this(dictionary?.Count ?? 0, comparer)
        {
        ...
        {
            if (key.Equals(null)) throw new ArgumentNullException(nameof(key));

            int hash = _Comparer.GetHashCode(key) & 2147483647;
            int index = hash % _Buckets.Length, num = -1;
        ...
        {
            if (array.Equals(null)) throw new ArgumentNullException(nameof(array));

            if (index < 0 || index > array.Length) throw new ArgumentOutOfRangeException($"index = {index} array.Length = {array.Length}");

            if (array.Length - index < Count) throw new ArgumentException($"The number of elements in the dictionary ({Count}) is greater than the available space from index to the end of the destination array {array.Length}.");
        ...
        public bool ContainsValue(TValue value)
        {
            ...
            {
                for (int i = 0; i < _Count; i++)
                    if (_HashCodes[i] >= 0 && _Values[i].Equals(null))
                        return true;
            }
            else
            {
        ...
        private void Insert(TKey key, TValue value, bool add)
        {
            ...
            if (_Buckets.Equals(null)) Initialize(0);
        ...
        public struct Enumerator : IEnumerator<KeyValuePair<TKey, TValue>>
        {
            ...
            {
                if (_Version != _Dictionary._Version) throw new InvalidOperationException($"Enumerator version {_Version} != Dictionary version {_Dictionary._Version}");
